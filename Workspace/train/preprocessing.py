...
def create_superpixels(img, method='slic', **kwargs):
    """
    Generate a superpixel segmentation of an image using a specified method.

    Parameters
    ----------
    img : ndarray
        Input RGB image of shape (H, W, C).
    method : str, optional
        Segmentation method to use: 'slic', 'felzenszwalb', 'watershed',
        'kmeans', or 'pixel'. Default is 'slic'.
    **kwargs
        Additional parameters passed to the selected segmentation algorithm.

    Returns
    -------
    ndarray
        A 2D array of shape (H, W) containing integer segment labels, where
        each unique label represents a distinct image region.
    """
